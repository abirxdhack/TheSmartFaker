...
This module provides country-specific IBAN (International Bank Account Number)
generation logic, including bank code data, country format specifications, and
async generator functions for each supported country.
...
Example::
    ...
    async def main():
        bban = await COUNTRY_GENERATORS["DE"]["generator"]()
        check = await calculate_check_digits("DE", bban)
        iban = f"DE{check}{bban}"
        valid = await validate_iban(iban)
        print(iban, valid)
...
#: Sample bank codes indexed by ISO 3166-1 alpha-2 country code.
bank_codes_data = {
    "AT": ["12000", "20151"],
    "AZ": ["NABZ", "AIIB"],
    "BH": ["BBKU", "AUBB"],
...
#: IBAN structure definitions per country: total length, field lengths, bank codes.
country_data = {
...
# ---------------------------------------------------------------------------
# Per-country BBAN generator coroutines
# ---------------------------------------------------------------------------
...
async def generate_br():
    """Generate Brazilian (BR) BBAN."""
    data = country_data["BR"]
    bank_code = random.choice(data["bank_codes"])
    branch_code = await generate_numeric(data["branch_code_length"])
    account_number = await generate_numeric(data["account_length"])
    account_type = await generate_alpha(data["account_type_length"])
    owner_type = await generate_alpha(data["owner_type_length"])
...
#: Registry mapping country codes to their IBAN length and async BBAN generator.
COUNTRY_GENERATORS = {
